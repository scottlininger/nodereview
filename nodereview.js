...
var promptSettings = {
  properties: {
    from: {
      description: 'Your gmail: (ENTER to preview only) ',
      // pattern: /^[a-zA-Z\s\-\@\.\_]+$/,
    ...
    password: {
      ...
      replace: '*',
      ask: function() {
        // only ask if from has value 
        return prompt.history('from').value.length > 0;
      }
    },
    to: {
...
prompt.get(promptSettings, function (err, promptResult) {
  ...
  exec('git show -U9000', { maxBuffer: tenMegabytes }, function (error, stdout, stderr) {
    ...
    // If they did not enter a valid email, then just preview locally.
    if (!promptResult.from) {
      openurl.open(REVIEW_LINK_URL + '#' + encodeURIComponent(showOutput));
      return;
    }
